...
def main(args):
    ...
    if args.hemi == "lh":
        ...
        svmtk_create_slice_mesh(stlfile=os.path.join(STLPATH, f"lh.pial{args.stlmod}.stl"), output=MSHMESH, plane=PLANE, resolution=args.resolution)

    elif args.hemi == "rh":
        print(f"Meshing right hemisphere ONLY.")
        svmtk_create_slice_mesh(stlfile=os.path.join(STLPATH, f"rh.pial{args.stlmod}.stl"), output=MSHMESH, plane=PLANE, resolution=args.resolution)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--prunez", action=argparse.BooleanOptionalAction, default=True, help="Save a copy of the mesh with the z-coordinates pruned. This is primarily for inspecting mesh quality. Default is True."
    )
