################################################################################
# 
# This script post-processes the single-node strong scaling study.
# 
# Usage: python3 pp_strong_scaling.py --log /path/to/logfile --outdir /path/to/output
# 
...
def generate_strong_figure(fname, nproc, p_ss, ideal_ss, obtained_ss):
    ...
    p2 = ax11.loglog(p_ss, obtained_ss, linestyle='--', marker='*', lw=1.05, ms=5, color=utorange)

    Y_OFFSET_OBTAINED = 0.6 # 0.8 for (12, 6) size
    ...
    MIDPOINT = 6  # hard-coded for CLX nodes
    
    ax11.text(MIDPOINT, MIDPOINT + 8, 'ideal strong scaling', color=charcoal, rotation=20)
    ax11.text(MIDPOINT, MIDPOINT - 4, 'obtained strong scaling', color=utorange, rotation=20)	
...
def main(args)->None:
    ...
    fname = f"{args.outdir}/strong_scaling.pdf"
    ...
    if args.adjoint is not None:
        ...
        fname = f"{args.outdir}/adjoint_strong_scaling.pdf"
        generate_strong_figure(fname, nproc, p_ss, ideal_ss, obtained_ss)
