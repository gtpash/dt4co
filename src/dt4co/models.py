...
class RDTumorVarf:
    ...
    def reaction(self):
        ...
        return kappa * self.u * (dl.Constant(1.0) - self.u) * self.p * self.dX
    ...
    def __call__(self, u, u_old, m, p, t):
        ...
        return (u - u_old) * p * self.dt_inv * self.dX + self.diffusion() - self.reaction()
...
class mollifiedRDTumorVarf(RDTumorVarf):
    ...
    def reaction(self):
        ...
        return kappa * moll * (dl.Constant(1.0) - self.u) * self.p * self.dX
...
class PWRDTumorVarf:
    ...
    def diffusion(self):
        D = ufl.exp(self.dg) * self.chi_gm + ufl.exp(self.dw) * (dl.Constant(1.0) - self.chi_gm)
        return D * ufl.inner(ufl.grad(self.u), ufl.grad(self.p)) * self.dX
...
class naiveRTxModel:
    ...
    def get_tx_factor(self, cur_t: float) -> dl.Constant:
        ...
        if cur_t in self.tx_days:
            ...
            return dl.Constant(0.0)  # no therapy. Radiotherapy effect is zero.
    ...
    def get_rt_sf(self, cur_t: float) -> dl.Constant:
        ...
        if cur_t in self.tx_days:
            ...
            return dl.Constant(1.0)  # no therapy. Surviving fraction is 1.
...
class naiveRTxTumorVarf:
    ...
    def reaction(self):
        kappa = ufl.exp(self.k)
        return kappa * self.u * (dl.Constant(1.0) - self.u) * self.p * ufl.dx
    ...
    def __call__(self, u, u_old, m, p, t):
        ...
        return (u - u_old) * p * self.dt_inv * ufl.dx + self.diffusion() - self.reaction()


class radioModel:
    """Class to compute the radiotherapy effect."""

    def __init__(self, tx_days: np.array, tx_doses: np.array, alpha: float = 3.0, alpha_beta_ratio: float = 10.0, m_width: float = 0.25):
        self.tx_days = tx_days  # therapy days
        self.tx_doses = tx_doses  # therapy doses
...
class RDchemoTumorVarf:
    ...
    def __call__(self, u, u_old, m, p, t):
        ...
        # be careful with the signs, we are in residual form LHS = 0
        return (u - u_old) * p * self.dt_inv * ufl.dx + self.diffusion() - self.reaction() + self.chemo(t)
...
class RDradioTumorVarf:
    ...
    def __call__(self, u, u_old, m, p, t):
        ...
        # be careful with the signs, we are in residual form LHS = 0
        return (u - u_old) * p * self.dt_inv * ufl.dx + self.diffusion() - self.reaction() + self.tx(t)
...
class RDTXTumorVarf:
    ...
    def __call__(self, u, u_old, m, p, t):
        ...
        # be careful with the signs, we are in residual form LHS = 0
        return (u - u_old) * p * self.dt_inv * ufl.dx + self.diffusion() - self.reaction() + self.radio(t) + self.chemo(t)
...
class mollifiedRDTXTumorVarf(RDTXTumorVarf):
    ...
    def __call__(self, u, u_old, m, p, t):
        ...
        # be careful with the signs, we are in residual form LHS = 0
        return (u - u_old) * p * self.dt_inv * self.dX + self.diffusion() - self.reaction() + self.radio(t) + self.chemo(t)
...
class CenteredPWRDTXTumorVarf:
    ...
    def diffusion(self):
        D = self.mdg0 * ufl.exp(self.dg) * self.chi_gm + self.mdw0 * ufl.exp(self.dw) * (dl.Constant(1.0) - self.chi_gm)
        return D * ufl.inner(ufl.grad(self.u), ufl.grad(self.p)) * self.dX
...
class DiffusionTumorVarf:
    ...
    def reaction(self):
        return self.kappa * self.u * (dl.Constant(1.0) - self.u) * self.p * self.dX
    ...
    def __call__(self, u, u_old, m, p, t):
        ...
        return (u - u_old) * p * self.dt_inv * self.dX - self.f * p * ufl.dx + self.diffusion() - self.reaction()
...
class ReactionTumorVarf:
    ...
    def reaction(self):
        K = ufl.exp(self.m)
        return K * self.u * (dl.Constant(1.0) - self.u) * self.p * self.dX
...
class LinearizedRDTumorVarf:
    ...
    def reaction(self, u_old):
        ...
        return kappa * u_old * (dl.Constant(1.0) - self.u) * self.p * ufl.dX
    ...
    def __call__(self, u, u_old, m, p, t):
        ...
        return (u - u_old) * p * self.dt_inv * ufl.dX - self.f * p * ufl.dx + self.diffusion() - self.reaction(u_old)
...
class HomogeneousMCRDTumorVarf:
    ...
    def __init__(self, dt, f, mesh, lmbdaf, D0, gamma, Ew, Eg, nu, chi, dim=3):
        ...
        # Material properties
        self.nu = nu
        self.mu = (Ew * self.chi + Eg * (1.0 - self.chi)) / (2 * (1 + self.nu))  # shear modulus
        self.lmbda = (Ew * self.chi + Eg * (1.0 - self.chi)) * self.nu / ((1 + self.nu) * (1 - 2 * self.nu))  # first lame parameter
    ...
    def stressDeviator(self, d):
        return self.sigma(d) - (1.0 / 3) * ufl.tr(self.sigma(d)) * ufl.Identity(self.dim)

    def vonMises(self, d):
        return ufl.sqrt(3.0 / 2 * ufl.inner(self.stressDeviator(d), self.stressDeviator(d)))
    ...
    def compute_diffusivity(self, u):
        ...
        bc = dl.DirichletBC(V, dl.Constant((0.0, 0.0, 0.0)), boundary)  # zero displacement on the boundary
    ...
    def __call__(self, u, u_old, m, p, t):
        ...
        return (u - u_old) * p * self.dt_inv * ufl.dx - self.f * p * ufl.dx + self.diffusion(D) - self.reaction()
...
class TDRealizationTracer(object):
    ...
    def append(self, current, q):
        ...
        self.par_mv[self.i].axpy(1.0, current.m)
        self.state_mv[self.i].axpy(1.0, current.u.view(self.tf))
