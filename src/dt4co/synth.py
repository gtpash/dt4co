...
def build_rt_timeline(start=7.0, num_weeks=6):
    tx_start = start
...
def build_ct_timeline(start=7.0, num_weeks=6):
    tx_start = start
...
class synthExperiment(Experiment):
    ...
    def trueParameter(self, Vh: list, sample: bool = False) -> dl.Vector:
        ...
        if sample:
            ...
            mprior = hp.VectorBiLaplacianPrior(
                Vh[hp.PARAMETER], [diff_coeffs[0], diff_coeffs[0], prolif_coeffs[0]], [diff_coeffs[1], diff_coeffs[1], prolif_coeffs[1]], mean=mu.vector(), robin_bc=True
            )
    ...
    def spoofContinuousMisfitTD(self, uh, visit_days, Vh: dl.FunctionSpace, noise_var: float) -> hp.MisfitTD:
        ...
        for i, visit_day in enumerate(visit_days):
            ...
            uhelp.vector().zero()
            uhelp.vector().axpy(1.0, uh.view(visit_day))
