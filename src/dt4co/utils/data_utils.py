...
def get_vox2ras(nii: nibabel.nifti1) -> np.ndarray:
    ...
    vox2ras = np.array([[ds[0], 0, 0, -ns[0]], [0, ds[1], 0, -ns[1]], [0, 0, ds[2], -ns[2]], [0, 0, 0, 1]], dtype=np.float32)
    return vox2ras
...
def applyFreeSurferShiftRAS(surfdir: str, surf: str, t1: str = None):
    ...
    header["cras"] = np.array([0.0, 0.0, 0.0])
...
def applyGaussianBlur(img: nibabel.nifti1, sigma: float = 1.0, clip: bool = True) -> nibabel.nifti1:
...
def applyInitialCondition(
    niifile: str, mesh: dl.Mesh, u: dl.Function, V: dl.FunctionSpace, useIntermediate: bool = False, enhanceval: float = 0.8, nonenhanceval: float = 0.16, use_vox2ras: bool = False
) -> None:
