...
class TestObservationOperator(unittest.TestCase):
    ...
    def test_Bstar(self):
        ...
        res.vector().zero()
        res.vector().axpy(1.0, xrbfun.vector())
        res.vector().axpy(-1.0, xfun.vector())
        assert res.vector().norm("linf") < 1e-6, "Unit direction x is not the same."

        res.vector().zero()
        res.vector().axpy(1.0, yrbfun.vector())
        res.vector().axpy(-1.0, yfun.vector())
        assert res.vector().norm("linf") < 1e-6, "Unit direction y is not the same."

        res.vector().zero()
        res.vector().axpy(1.0, zrbfun.vector())
        res.vector().axpy(-1.0, zfun.vector())
