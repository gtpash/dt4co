...
def main(args)->None:
    ...
    noisy_utrue = utrue.copy()
    if args.noisy:
        hp.parRandom.normal_perturb(exp.NOISE, noisy_utrue)
        
    pde.exportState(noisy_utrue, os.path.join(OUT_DIR, "noisy_state.xdmf"))
    
    misfits = exp.spoofContinuousMisfitTD(uh=utrue, visit_days=visit_days, Vh=Vh[hp.STATE], noise_var=NOISE_VAR)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mollic", action=argparse.BooleanOptionalAction, default=False, help="Use mollified initial condition.")
